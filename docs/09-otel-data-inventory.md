...
| `source` (`config` / `user_permanent` / ...) | ✅ | ❌ | 許可経路別の可視化は未実装 |

### 3.5 `api_error` — API リクエスト失敗

| フィールド | 収集 | 可視化 | 備考 |
|---|---|---|---|
| `model` | ✅ | ❌ | モデル別エラー内訳の可視化は未実装 |
| `error` | ✅ | ✅ | API エラー詳細一覧（activity-analysis） |
| `status_code` | ✅ | ✅ | 〃 |
| `duration_ms` | ✅ | ❌ | エラー時レイテンシの可視化は未実装 |
| `attempt` | ✅ | ❌ | リトライ回数の可視化は未実装 |
...
| フィールド | 収集 | 可視化 | 備考 |
|---|---|---|---|
| `action` (`login` など) | ✅ | ❌ | 認証アクション別集計は未実装 |
| `success` | ✅ | ✅ | 認証エラー数（activity-analysis） |
| `auth_method` | ✅ | ❌ | 認証方式別の可視化は未実装 |
...
| `transport_type` (`stdio` / `http`) | ✅ | ❌ | トランスポート種別の可視化は未実装 |
| `server_scope` (`local` / `user`) | ✅ | ❌ | — |
| `duration_ms` | ✅ | ❌ | 接続レイテンシの可視化は未実装 |
...
| `skill.source` (`bundled` など) | ✅ | ❌ | ソース別の可視化は未実装 |
...
- ツール品質（ツール accept 率 / 成功率 / 実行数 / 平均・最大実行時間 / 失敗数 / ツール別 accept/reject 件数 / ツール別平均実行時間）
- プロンプト分析（総プロンプト数 / 平均プロンプト長 / 日次プロンプト数推移）
- エラー監視（API エラー数 / 内部エラー数 / 認証エラー数 / API エラー詳細一覧）
- MCP・Skills・Hooks（MCP 接続数 / スキル起動数 / フック成功率 / フックイベント別 成功/失敗件数 / スキル別起動件数）

### 6.2 収集しているが **可視化していない**（追加可視化の候補）

**ログイベント系**
- `tool_result_size_bytes` → ペイロードサイズ分布
- `tool_decision.source` → 許可経路別（config / user_permanent など）内訳
- `api_error.model` / `attempt` → モデル別エラー率・リトライ回数
- `api_error.duration_ms` → エラー時レイテンシ
- `auth.action` / `auth.auth_method` → 認証アクション・方式別集計
- `mcp_server_connection.transport_type` / `duration_ms` → トランスポート種別・接続レイテンシ
- `skill_activated.skill.source` → Skill ソース別内訳
- `hook_execution_*.hook_name` / `total_duration_ms` → Hook 名別・レイテンシ監視
